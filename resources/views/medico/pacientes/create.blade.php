...
<div class="min-h-screen flex justify-center items-start py-10 bg-gray-50">
    <div class="bg-white shadow-xl rounded-2xl w-full max-w-2xl border border-gray-200 p-10">
        ...
        <div class="text-center mb-10">
            ...
            <p class="text-gray-500 mt-1">Por favor, ingrese los datos del paciente</p>
        </div>
        ...
        <form action="{{ route('medico.pacientes.store') }}" method="POST" class="space-y-6 text-gray-800">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="segundo_nombre" value="{{ old('segundo_nombre') }}" required
                           class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const oldParroquia = "{{ old('parroquia') }}";
...
cedulaInput.addEventListener('input', function() {
    ...
    if (cedula.length === 10) {
        fetch(`/medico/pacientes/validar-cedula/${cedula}`)
            .then(res => res.json())
            .then(data => {
                mensajeCedula.textContent = data.mensaje;
                mensajeCedula.className = data.valido ? 'text-green-600 text-sm mt-1' : 'text-red-600 text-sm mt-1';
            })
            .catch(() => {
                mensajeCedula.textContent = 'Error validando la cédula.';
                mensajeCedula.className = 'text-red-600 text-sm mt-1';
            });
    } else if (cedula.length > 0) {
        mensajeCedula.textContent = 'Debe tener 10 dígitos numéricos.';
        mensajeCedula.className = 'text-red-600 text-sm mt-1';
    } else {
