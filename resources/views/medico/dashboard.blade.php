...
<body class="bg-gray-100 min-h-screen font-sans">
  ...
  <header class="bg-blue-700 text-white py-4 shadow-md">
    <div class="container mx-auto flex justify-between items-center px-6">
      <div class="flex items-center gap-3">
        <div class="h-9 w-9 rounded-xl bg-white/15 flex items-center justify-center font-bold">008</div>
        <div>
          <h1 class="text-lg sm:text-xl font-semibold leading-tight">Sistema de Emergencias - Hospital de Jipijapa</h1>
          <p class="text-xs text-white/80">Módulo Médico</p>
        </div>
      </div>
      ...
      <div class="flex items-center space-x-4">
        ...
        <form method="POST" action="{{ route('logout') }}">
          @csrf
          <button type="submit" class="bg-blue-500 hover:bg-blue-600 px-3 py-2 rounded-lg text-sm font-semibold">
            Salir
          </button>
        </form>
      </div>
  ...
  <main class="max-w-7xl mx-auto p-6 space-y-6">
    ...
    <!-- Hero -->
    <section class="rounded-2xl bg-white border shadow-sm p-6">
