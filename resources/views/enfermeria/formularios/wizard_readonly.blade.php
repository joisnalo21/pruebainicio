    ...
    <div class="py-6 bg-gray-50 min-h-screen">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm rounded-2xl p-4 mb-6">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div class="text-xs px-2 py-1 rounded-full
                        {{ $formulario->esCompleto() ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-800' }}">
                        {{ $formulario->esCompleto() ? 'COMPLETO' : 'BORRADOR' }}
                    </div>
                </div>
                ...
                <div class="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-13 gap-2">
                    @foreach($steps as $n => $label)
                        ...
                        <a
                            href="{{ route('enfermero.formularios.ver.paso', ['formulario' => $formulario->id, 'paso' => $n]) }}"
            ...
            <div class="mb-4 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-blue-900">
                Este formulario se muestra en <span class="font-semibold">modo consulta</span>. Los campos están bloqueados.
            </div>

            {{-- Contenido del paso (re-usa las mismas vistas) --}}
